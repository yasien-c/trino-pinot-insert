        ...
        implements RowDecoderFactory
{
    ...
    public static final String SCHEMA_REGISTRY_CAPACITY_KEY = "schemaRegistryCapacity";

    @Override
    public RowDecoder create(Map<String, String> decoderParams, Set<DecoderColumnHandle> columns)
    {
        checkState(decoderParams.containsKey(SCHEMA_REGISTRY_URL_KEY), "Missing required parameter '%s'", SCHEMA_REGISTRY_URL_KEY);
        checkState(decoderParams.containsKey(SCHEMA_REGISTRY_CAPACITY_KEY), "Missing required parameter '%s'", SCHEMA_REGISTRY_CAPACITY_KEY);
        ...
        SchemaRegistryClient schemaRegistryClient = new CachedSchemaRegistryClient(decoderParams.get(SCHEMA_REGISTRY_URL_KEY), Integer.valueOf(decoderParams.get(SCHEMA_REGISTRY_CAPACITY_KEY)));
