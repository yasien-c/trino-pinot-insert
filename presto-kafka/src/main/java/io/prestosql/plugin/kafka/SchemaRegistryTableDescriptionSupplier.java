...
import static io.prestosql.plugin.kafka.KafkaSplitManager.isURI;
import static java.nio.file.Files.readAllBytes;
import static java.util.Arrays.asList;
import static java.util.Objects.requireNonNull;

public class SchemaRegistryTableDescriptionSupplier
        implements Supplier<Map<SchemaTableName, KafkaTopicDescription>>
{
    ...
    private static final Logger log = Logger.get(SchemaRegistryTableDescriptionSupplier.class);
    ...
    @Inject
    SchemaRegistryTableDescriptionSupplier(KafkaConnectorConfig kafkaConnectorConfig,
                                           JsonCodec<KafkaTopicDescription> topicDescriptionCodec)
    {
        ...
        checkArgument(kafkaConnectorConfig.getSchemaRegistry().isPresent(), "schemaRegistry is not present");
        checkArgument(isURI(kafkaConnectorConfig.getSchemaRegistry().get()), "schemaRegistry is not a valid URI");
        try {
            this.schemaRegistryUrl = new URL(kafkaConnectorConfig.getSchemaRegistry().get());
        }
    ...
    public Map<SchemaTableName, KafkaTopicDescription> get()
    {
        ...
        log.debug("Loading kafka table definitions from %s", schemaRegistryUrl);
