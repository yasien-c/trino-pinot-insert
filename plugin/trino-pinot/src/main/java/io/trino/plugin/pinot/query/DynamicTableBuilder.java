...
public final class DynamicTableBuilder
{
    ...
    private static Map<String, PinotColumnNameAndTrinoType> getAggregateTypes(SchemaTableName schemaTableName, QueryContext queryContext, Map<String, ColumnHandle> columnHandles)
    {
        ...
        // Note: the column name is set by the postaggregation schema
        ...
        for (int index = 0; index < postAggregtionSchema.size(); index++) {
            aggregationTypesBuilder.put(rewriteExpression(schemaTableName, aggregateColumnExpressions.get(index), columnHandles).toString(), new PinotColumnNameAndTrinoType(postAggregtionSchema.getColumnName(index), toTrinoType(postAggregtionSchema.getColumnDataType(index))));
        }
