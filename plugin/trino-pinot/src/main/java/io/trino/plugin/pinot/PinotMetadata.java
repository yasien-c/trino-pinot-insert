        ...
        implements ConnectorMetadata
{
    ...
    private boolean isFilterPushdownUnsupported(Domain domain)
    {
        ...
        return (valueSet.isAll() && !domain.isNullAllowed()) ||
                (valueSet.isNone() && (domain.isNullAllowed() || !SUPPORTS_ALWAYS_FALSE.contains(domain.getType()))) ||
                (!valueSet.getRanges().getOrderedRanges().isEmpty() && domain.isNullAllowed());
    }
