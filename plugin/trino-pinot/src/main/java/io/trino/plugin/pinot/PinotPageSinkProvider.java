        ...
        implements ConnectorPageSinkProvider
{
    ...
    private final Optional<PinotDeepStore> pinotDeepStore;
    private final ExecutorService segmentBuilderExecutor;
    private final ListeningExecutorService metadataPushExecutor;

    @Inject
    public PinotPageSinkProvider(PinotConfig config, PinotClient pinotClient, Optional<PinotDeepStore> pinotDeepStore)
    {
        ...
        metadataPushExecutor = listeningDecorator(newCachedThreadPool(daemonThreadsNamed("pinot-metadata-push-%s")));
        segmentBuilderExecutor = new ThreadPoolExecutor(config.getSegmentBuilderParallelism(), config.getSegmentBuilderParallelism(), 0, MILLISECONDS, new TimedBlockingQueue<>(config.getSegmentBuilderQueueSize(), config.getSegmentBuilderQueueTimeout().toMillis()), daemonThreadsNamed("pinot-segment-builder-%s"));
    }
