    ...
    public RowDecoder create(Map<String, String> decoderParams, Set<DecoderColumnHandle> columns)
    {
        ...
        switch (parsedSchema.getType()) {
            case LONG:
                ...
                break;
            case INT:
                // The TimeMillis logical type requires an avro int but trino requires picos (long type) to construct SqlTime
                if (parsedSchema.getLogicalType() instanceof TimeMillis) {
                    avroDeserializer = data -> LONG_DESERIALIZER.deserialize(null, data);
                }
                else {
