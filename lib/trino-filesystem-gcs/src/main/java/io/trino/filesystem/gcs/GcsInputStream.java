        ...
        extends TrinoInputStream
{
    ...
    public GcsInputStream(GcsLocation location, Blob blob, int readBlockSizeBytes)
            throws IOException
    {
        this.location = requireNonNull(location, "location is null");
        this.blob = requireNonNull(blob, "blob is null");
        this.readBlockSizeBytes = readBlockSizeBytes;
        this.fileSize = requireNonNull(blob.getSize(), "blob size is null");
        openStream();
    }
    ...
    {
        try {
            this.readChannel = getReadChannel(blob, location, 0L, readBlockSizeBytes);
        }
        catch (RuntimeException e) {
