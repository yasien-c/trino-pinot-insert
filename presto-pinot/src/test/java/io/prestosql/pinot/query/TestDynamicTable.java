        ...
        extends TestPinotQueryBase
{
    ...
    public void testSelectNoFilter()
    {
        ...
        List<OrderByExpression> orderByExpressions = orderByColumns.stream()
                ...
                .collect(toList());
        long limit = 230;
        String pql = format("select %s from %s order by %s limit %s",
                columnNames.stream()
                        .collect(joining(", ")),
                tableName,
    ...
    public void testGroupBy()
    {
        String tableName = realtimeOnlyTable.getTableName();
        long limit = 25;
        String pql = format("SELECT max(CarrierDelay), avg(CarrierDelay) FROM %s GROUP BY Origin, AirlineID TOP %s", tableName, limit);
        DynamicTable dynamicTable = buildFromPql(pinotMetadata, new SchemaTableName("default", pql));
        ...
        assertEquals(dynamicTable.getAggregateColumns(), ImmutableList.builder()
                .add(new AggregationExpression("max_CarrierDelay", "CarrierDelay", "max"))
                .add(new AggregationExpression("avg_CarrierDelay", "CarrierDelay", "avg"))
                .build());
        assertEquals(dynamicTable.getLimit().getAsLong(), limit);
    }
    ...
    {
        String tableName = realtimeOnlyTable.getTableName();
        String pql = format("select FlightNum, AirlineID from %s where ((CancellationCode IN ('strike', 'weather', 'pilot_bac')) AND (Origin = 'jfk')) " +
        ...
        DynamicTable dynamicTable = buildFromPql(pinotMetadata, new SchemaTableName("default", pql));
        assertEquals(extractPql(dynamicTable, TupleDomain.all(), ImmutableList.of()), pql);
    }
    ...
    {
        String tableName = realtimeOnlyTable.getTableName();
        String pql = format("select * from %s limit 70", tableName.toLowerCase(ENGLISH));
        DynamicTable dynamicTable = buildFromPql(pinotMetadata, new SchemaTableName("default", pql));
