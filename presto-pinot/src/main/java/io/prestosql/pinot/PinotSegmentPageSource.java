...
import io.prestosql.pinot.client.PinotScatterGatherQueryClient.ErrorCode;
        ...
        implements ConnectorPageSource
{
    ...
    private static void checkExceptions(DataTable dataTable, PinotSplit split, String pql)
    {
    ...
    private void fetchPinotData()
    {
        ...
        try {
            ...
            dataTableMap.values().stream()
                    ...
                    .forEach(dataTable ->
                    {
                        checkExceptions(dataTable, split, pql);
    ...
    private Map<ServerInstance, DataTable> queryPinot()
    {
        ...
        try {
            log.info("Query pql  '%s' on host '%s' for segment splits: %s", pql, split.getSegmentHost(), split.getSegments());
